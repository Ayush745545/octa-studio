...
export async function POST(request: Request) {
  try {
    ...
    if (file.type.startsWith("video/")) {
      ...
      try {
        // Prefer re-encoding with audio. Some sources (e.g. screen
        // recordings, silent clips) have no audio stream, which makes the
        // `-c:a aac` pass fail — retry without audio so we still produce a
        // browser-playable MP4 instead of keeping an unplayable original.
        try {
          await encode(["-c:a", "aac"]);
        } catch (audioError) {
          console.warn(
            "FFmpeg audio encode failed; retrying without audio:",
            audioError,
          );
          await encode([]);
        }
